import streamlit as st 
import pandas as pd
import numpy as np
import pickle

# Load data and similarity matrix
electronics = pickle.load(open('data.pkl', 'rb'))
similarity = pickle.load(open('similarity.pkl', 'rb'))

# Function to get product recommendations
def recommendation(product):
    electronics_index = electronics[electronics['name'] == product].index[0]
    similarity_list = list(enumerate(similarity[electronics_index]))
    top_3_similar_product = sorted(similarity_list, key=lambda x: x[1], reverse=True)[1:4]
    
    similar_product = []
    for idx, similar_score in top_3_similar_product:
        if pd.notna(electronics.loc[idx]['image']) and electronics.loc[idx]['image']:  # Check if image is present
            product_info = {
                'name': electronics.loc[idx]['name'],
                'image': electronics.loc[idx]['image'],
            }
            similar_product.append(product_info)
    return similar_product

# Streamlit app
st.title('Product Recommendation System')

product_name = st.selectbox('Select a product:', electronics['name'])

if st.button('Recommend'):
    search_product = recommendation(product_name)

    # Create columns for displaying the products in a row
    cols = st.columns(len(search_product))  # Adjust number of columns based on the number of similar products
    for col, rec in zip(cols, search_product):
        with col:
            st.image(rec['image'], width=150)
            st.write(f"**{rec['name']}**")

def set_bg_from_url(url, opacity=1):
        
        footer = """
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-gH2yIJqKdNHPEq0n4Mqa/HGKIhSkIHeL5AyhkYV8i59U5AR6csBvApHHNl/vI1Bx" crossorigin="anonymous">
        <footer>
            <div style='visibility: visible;margin-top:7rem;justify-content:center;display:flex;'>
                <p style="font-size:1.1rem;">
                    Made by Mohamed Shaad
                    &nbsp;
                    <a href="https://www.linkedin.com/in/mohamedshaad">
                        <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="white" class="bi bi-linkedin" viewBox="0 0 16 16">
                            <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
                        </svg>          
                    </a>
                    &nbsp;
                    <a href="https://github.com/shaadclt">
                        <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="white" class="bi bi-github" viewBox="0 0 16 16">
                            <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
                        </svg>
                    </a>
                </p>
            </div>
        </footer>
    """
        st.markdown(footer, unsafe_allow_html=True)
        
        
        # Set background image using HTML and CSS
        st.markdown(
            f"""
            <style>
                body {{
                    background: url('{url}') no-repeat center center fixed;
                    background-size: cover;
                    opacity: {opacity};
                }}
            </style>
            """,
            unsafe_allow_html=True
        )

# Set background image from URL
set_bg_from_url("https://img.freepik.com/free-vector/wearable-technology-isometric-flowchart_1284-19018.jpg?w=740&t=st=1723049490~exp=1723050090~hmac=2c52cc76824ba2a41475065bfe62e7f2444cbf92e98fa242dee67ab7242e2b26", opacity=0.875)



